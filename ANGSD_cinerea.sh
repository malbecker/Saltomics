...
for i in $SAMPLES; do
    ...
    ${WD}/rcorr/${i}.TRIM_2P.cor.fq > ${WD}/bamfiles/${i}.sam
done
...
### global angsd!
$ANGSD -b samples4angsd.txt -anc $TRANSCRIPTOME -out angsd_global -P 24 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -setMinDepth 100 -setMaxDepth 6000 -minInd 6 -minMaf 0.01 -GL 1 -doMaf 1 -doMajorMinor 1 -doGlf 3 -doPost 1 -doVcf 1 -doCounts 1 -doDepth 1 -dumpCounts 1 -dosaf 1
...
# run angsd on coastal and inland frogs
grep -E 'BL|LO|PWL|INL' samples4angsd.txt > inland4angsd.txt
grep -E 'BOD1|BOD2|CSI|DQ' samples4angsd.txt > coastal4angsd.txt

# inland
$ANGSD -b inland4angsd.txt -anc $TRANSCRIPTOME -out inland_angsd -P 24 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -setMinDepth 100 -setMaxDepth 6000 -minInd 6 -minMaf 0.01 -GL 1 -doMaf 1 -doMajorMinor 1 -doGlf 3 -doPost 1 -doVcf 1 -doCounts 1 -doDepth 1 -dumpCounts 1 -dosaf 1

# coastal
$ANGSD -b coastal4angsd.txt -anc $TRANSCRIPTOME -out coastal_angsd -P 24 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -setMinDepth 100 -setMaxDepth 6000 -minInd 6 -minMaf 0.01 -GL 1 -doMaf 1 -doMajorMinor 1 -doGlf 3 -doPost 1 -doVcf 1 -doCounts 1 -doDepth 1 -dumpCounts 1 -dosaf 1

# run angsd on each individual population
pops=$(ls ${WD}/rcorr/*TRIM_1P.cor.fq | cut -d _ -f 3 | sort | uniq)
for pop in $pops
do
grep ${pop} samples4angsd.txt > ${pop}4angsd.txt
done

for pop in $pops
do
$ANGSD -b ${pop}4angsd.txt -anc $TRANSCRIPTOME -out ${pop}_angsd -P 24 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -setMinDepth 50 -setMaxDepth 6000 -minInd 2 -minMaf 0.01 -GL 1 -doMaf 1 -doMajorMinor 1 -doGlf 3 -doPost 1 -doVcf 1 -doCounts 1 -doDepth 1 -dumpCounts 1 -dosaf 1
done

### Calculate fst

#calculate the 2dsfs prior
SFS=$"/mnt/lustre/macmaneslab/ams1236/software/angsd/misc/realSFS"
$SFS coastal_angsd.saf.idx inland_angsd.saf.idx  > coastal.inland.ml

#prepare the fst for easy window analysis etc
$SFS fst index coastal_angsd.saf.idx -sfs coastal.inland.ml -fstout coastalinlandcompare

#get the global estimate
$SFS fst stats coastalinlandcompare.fst.idx 

#below is not tested that much, but seems to work
$SFS fst stats2 coastalinlandcompare.fst.idx  -win 50000 -step 10000 > slidingwindow
